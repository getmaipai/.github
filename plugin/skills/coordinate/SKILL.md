...
**A Home change is proven on 8787, never on a worktree's build (2026-09-21).** The owner looks at `localhost:8787`, served from the main checkout; a landing is not seen there until the session has pulled main fast-forward, run `bun install --force` in `frontend/` and `backend/` (a pin bump is otherwise invisible: `bun restart` never installs, and one afternoon 8787 served the single-root fix with the previous tag's palette for an hour), and run `bun restart`. The done report's "what the owner sees" line is then a probe or a screenshot taken from 8787 itself. When a page needs a signed-in session the sessions cannot get (the owner's real household, no seeded account), the coordinator probes it through the owner's own signed-in browser tab (the Chrome tools), never by asking the owner for a secret and never by a session guessing one.
